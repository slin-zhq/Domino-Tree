...
This module removes per-node kernel-launch overhead from the DominoTree
conditional build stage. The best-first heap in ``dominotree.py`` stays in
Python (its pops are data-dependent, so the whole loop cannot be captured);
only the fixed-shape per-node GPU compute inside ``children_fn`` is replaced
by a CUDA-graph replay, following the static-buffer capture/replay pattern of
Domino's released ``DraftCorrectionGraphRunner``
(``ref Domino code/kernel/domino.py`` + its use in ``dflash.py:spec_generate``):

    copy inputs into static buffers -> graph.replay() -> read static outputs

Equivalence by construction
---------------------------
``GraphNodeExpander.children_fn`` mirrors
``domino_adapter.make_conditional_children_fn`` op-for-op, on identical
shapes/dtypes; the graph bodies below are line-by-line transcriptions where
the only change is that inputs are read from static buffers (values are
bit-identical copies) and depth-dependent tensors are selected with
``index_select`` on a static depth-index tensor instead of Python-side
indexing. The same body functions run either eagerly (``use_graphs=False``,
used by the CPU self-test and as a GPU debugging fallback via
``DOMINOTREE_GPU_EAGER=1``) or inside the captured graphs, so the captured
math is exactly what the self-test verifies against the pure-Python path.

Three graphs are captured:

* setup graph (once per round, ``corr_topm > 0`` only): per-depth marginal
  top-M candidate ids, their fc2 weight rows, and their base logits.
* corr graph (once per node pop at depth >= prefix_len): GRU-state
  correction -> corrected top-``node_topk`` tokens/logprobs -> ``node_topk``
  child GRU states.
* base-expand graph (once per node pop at depth < prefix_len): child GRU
  states for the (path-independent) per-depth base top-k tokens.

All performance characteristics are TO BE MEASURED ON GPU; this module was
authored and CPU-tested without CUDA hardware. Run the equivalence suite with

    python dominotree_gpu.py      (or ``python dominotree.py``, which calls it)

On a CUDA machine the suite additionally captures/replays the graphs on tiny
models and checks them against the pure-Python reference.
"""
...
class GraphNodeExpander:
    ...
    Construct once per process (graph capture is expensive), call
    ``begin_round(ph, base_logits)`` once per draft block, then pass
    ``expander.children_fn`` to ``dominotree.build_best_first_tree`` in place
    of ``domino_adapter.make_conditional_children_fn(...)``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if use_graphs is None:
            use_graphs = device.type == "cuda" and os.environ.get("DOMINOTREE_GPU_EAGER", "0") != "1"
        ...
        if self.corr_topm > 0:
            self.S_cand_all = torch.zeros(self.k_draft, self.corr_topm, dtype=torch.long, device=device)
            self.S_w2c_all = torch.zeros(self.k_draft, self.corr_topm, mlp_dim, dtype=torch.float32, device=device)
            self.S_basec_all = torch.zeros(self.k_draft, self.corr_topm, dtype=torch.float32, device=device)
        ...
        self.S_out_states = torch.zeros(1, self.node_topk, gru_dim, dtype=dtype, device=device)
        # ---- pinned host landing buffers (one sync per pop) ----
        if device.type == "cuda":
            self.H_tokens = torch.zeros(self.node_topk, dtype=torch.long, pin_memory=True)
            self.H_lps = torch.zeros(self.node_topk, dtype=torch.float32, pin_memory=True)

        # (device index tensor, token list, logprob list) per depth < prefix_len
        self._prefix_rows: list[tuple[torch.Tensor, list[int], list[float]]] | None = None
    ...
    # domino_adapter.make_conditional_children_fn line-by-line.
    # ------------------------------------------------------------------

    def _setup_impl(self) -> None:
        """Per-round corr_topm candidate setup (mirrors the adapter's cand/w2c/basec)."""
        w2 = self.draft.embed_proj[2].weight
        for d in range(self.k_draft):
            row = self.S_base_all[d]
            cd = torch.topk(row, self.corr_topm).indices  # == topk(base_logits[d], corr_topm).indices
            self.S_cand_all[d].copy_(cd)
            self.S_w2c_all[d].copy_(w2[cd].float())  # == embed_proj[2].weight[cand[d]].float()
            self.S_basec_all[d].copy_(row[cd].float())  # == base_logits[d][cand[d]].float()
    ...
    def _corr_impl(self) -> None:
        ...
        s_feat = self.S_state.transpose(0, 1).squeeze(0)  # == state.transpose(0, 1).squeeze(0)
        ...
        vals, local_idx = torch.topk(torch.log_softmax(corrected, dim=-1), k=self.node_topk)
    ...
    def _corr_full_impl(self) -> None:
        ...
        base_d = self.S_base_all.index_select(0, self.S_depth)  # == base_logits[depth].unsqueeze(0)
    ...
    def _base_expand_impl(self) -> None:
        """Child-state expansion for depth < prefix_len (tokens supplied via S_exp_tokens)."""
        self._expand_impl(self.S_exp_tokens)

    def _expand_impl(self, toks: torch.Tensor) -> None:
        emb = self.target.model.embed_tokens(toks).unsqueeze(1)  # (node_topk, 1, E)
    ...
    # ------------------------------------------------------------------
    # Capture (Domino DraftCorrectionGraphRunner pattern: warm up the exact
    # bodies on a side stream, then record each into a CUDAGraph).
    # ------------------------------------------------------------------
    ...
    def begin_round(self, ph: torch.Tensor, base_logits: torch.Tensor) -> None:
        ...
        if ph.dtype != self.S_ph_all.dtype or base_logits.dtype != self.S_base_all.dtype:
            raise TypeError(
        ...
        for d in range(self.prefix_len):
            # == domino_adapter.log_prob_topk(base_logits[d], node_topk), which the
            # Python path re-evaluates per node; it is path-independent, so once
            # per round is value-identical.
    ...
    def children_fn(self, state, depth: int):
        """Drop-in ``ChildrenFn`` for ``dominotree.build_best_first_tree``."""
...
class _TinyDraft(torch.nn.Module):
    ...
    def __init__(self, hidden: int, gru_dim: int, mlp_dim: int, vocab: int) -> None:
        ...
        self.prefix_gru = torch.nn.GRU(
            input_size=hidden, hidden_size=gru_dim, num_layers=1, batch_first=True, bias=False
        )
...
def _equivalence_suite(device: torch.device) -> None:
    ...
    with torch.no_grad():
        # (corr_topm, prefix_len) configs: reported public config analogue,
        # no-prefix, full-vocab correction, full-vocab with deeper prefix, and
        # all-prefix (corr graph never popped).
        for corr_topm, prefix_len in [(8, 1), (8, 0), (0, 0), (0, 2), (3, 5)]:
            ...
            for round_idx in range(2):  # two rounds: exercises static-buffer reuse
                gen = torch.Generator().manual_seed(1000 * round_idx + corr_topm * 10 + prefix_len)
                ...
                ref_nodes = dominotree.build_best_first_tree(ref_fn, root_state, budget, k_draft)

                expander.begin_round(ph, base_logits)
                fast_nodes = dominotree.build_best_first_tree(expander.children_fn, root_state, budget, k_draft)

                key = f"corr_topm={corr_topm} prefix_len={prefix_len} round={round_idx} device={device}"
                assert len(ref_nodes) == len(fast_nodes), key
                for a, b in zip(ref_nodes, fast_nodes):
                    assert (a.token, a.depth, a.parent) == (b.token, b.depth, b.parent), (key, a, b)
...
def _self_test() -> None:
    ...
    print("dominotree_gpu self-test (cpu, eager-static vs pure-Python): ALL PASSED")
    if torch.cuda.is_available():
        _equivalence_suite(torch.device("cuda"))
        print("dominotree_gpu self-test (cuda, CUDA-graph replay vs pure-Python): ALL PASSED")
    else:
        print("dominotree_gpu self-test: no CUDA device; graph capture/replay NOT exercised here")
