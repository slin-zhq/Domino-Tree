...
## Provenance
Collected 2026-07-09 single-GPU (sibling idle) — 4B on a dual-5080 node (GPU0, GPU1 idle),
8B on an A6000 (single-GPU by construction) — because Domino's CUDA-graph runner is sensitive to
host-side contention. Reproduce the derived rows with `scripts/domino_tree/build_domino_rows.py`
and `pairwise_domino_recompute.py` (in the SpecDec-Optimize repo).
